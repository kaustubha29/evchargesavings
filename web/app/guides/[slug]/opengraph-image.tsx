...
import { getGuideBySlug } from "@/features/guides/data";
...
export default async function Image({ params }: Props) {
  ...
  const title = guide.title.length > 65 ? guide.title.slice(0, 62) + "…" : guide.title;
  const sub   = (guide.hook ?? guide.description).length > 95
    ? (guide.hook ?? guide.description).slice(0, 92) + "…"
    : (guide.hook ?? guide.description);
      ...
      <div style={{ display: "flex", width: "100%", height: "100%", backgroundColor: "#081510", fontFamily: "sans-serif", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 80px 52px", width: "100%" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 36 }}>
            <span style={{ display: "flex", fontSize: 19, color: "#2ecc71" }}>⚡</span>
            <span style={{ display: "flex", fontFamily: "monospace", fontSize: 12, letterSpacing: "0.16em", textTransform: "uppercase", color: "rgba(245,240,232,0.35)" }}>EV Charge Savings</span>
            <span style={{ display: "flex", fontFamily: "monospace", fontSize: 12, color: "rgba(46,204,113,0.35)", letterSpacing: "0.08em" }}>·</span>
            <span style={{ display: "flex", fontFamily: "monospace", fontSize: 12, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(46,204,113,0.6)" }}>Guide</span>
            <span style={{ display: "flex", fontFamily: "monospace", fontSize: 12, color: "rgba(245,240,232,0.22)", letterSpacing: "0.08em", marginLeft: 10 }}>{guide.category}</span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, backgroundColor: "rgba(46,204,113,0.12)", border: "1px solid rgba(46,204,113,0.28)", borderRadius: 100, padding: "8px 18px" }}>
              ...
              <span style={{ display: "flex", fontFamily: "monospace", fontSize: 12, color: "#2ecc71", letterSpacing: "0.06em" }}>{guide.readTime}</span>
            </div>
            <span style={{ display: "flex", fontFamily: "monospace", fontSize: 13, color: "rgba(245,240,232,0.18)", letterSpacing: "0.08em" }}>evchargesavings.com/guides</span>
          </div>
        </div>
