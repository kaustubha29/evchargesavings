...
export function StatCard({ label, value, sub, accent, className }: Props) {
  return (
    <div className={cn("stat-card flex flex-col gap-1", accent && "border-emerald-bright ring-1 ring-emerald-bright", className)}>
      <div className="font-mono text-[9px] sm:text-[10px] uppercase tracking-widest text-ink-mute">{label}</div>
      <div className={cn("font-serif text-xl sm:text-3xl font-medium tracking-tight leading-none", accent ? "text-forest" : "text-ink")}>
        {value}
      </div>
